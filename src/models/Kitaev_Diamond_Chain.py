...
class Kitaev_Diamond_Chain(Hamiltonian):
    def __init__(self, Lat, Para):
        ...
        self.Ham = self.BuildDiamondChain()


    def BuildDiamondChain(self):
        """
        Build Kitaev Dimer Chain Hamiltonian
        1 - x - 3       5 - x - 7       9 - x - 11 
        | \     | \     | \     | \     | \     |  
        ...
        """
        for i in np.arange(0, self.Nsite):
            # Kxx_Conn
            j = self.Lat.nn_[i, 0]
            if j >= 0:
        ...
        # --------------------------- Add flux bias -------------------------
        if self.muFlux != 0.0:
            print("Adding flux bias to Hamiltonian, muFlux = ", self.muFlux)
            ws1_indx, ws1_ops, ws2_indx, ws2_ops, _, _ = self.buildFluxList()
            w1s = self.BuildFlux(ws1_indx, ws1_ops)
            w2s = self.BuildFlux(ws2_indx, ws2_ops)
            for w1 in w1s:
                Ham += w1 * self.muFlux
            for w2 in w2s:
                Ham += w2 * self.muFlux
    ...
    def buildFluxList(self):
        ...
        0 - x - 2       4 - x - 6       8 - x - 10     
        with 
        Ws1 = Z0 X1 Y2 + translations
        Ws2 = Y1 X2 Z3 + translations
        Ws1.Ws2 = Wp = Z0 Z1 Z2 Z3 + translations
        """
        print("Building flux list...")
        ...
        for i in np.arange(0, self.Lat.Nsite, 2):
            ...
            if i % 4 == 0:
                ...
                j1 = self.Lat.nn_[i, 1]
                j2 = self.Lat.nn_[i, 0] 
                print("[Ws1 sites] j0, j1, j2 = ", j0, j1, j2)
                if j0 >= 0 and j1 >= 0 and j2 >= 0:
                    Ws1_Indx.append([j0, j1, j2])
                    Ws1_Ops.append(["z", "x", "y"])  # Z0 X1 Y2, the order of operators in the plaquette
                # Wp operator with lower-left corner on sites 0, 4, 8, ...
                j3 = self.Lat.nn_[j2, 1] 
                print("[Wp sites] j0, j1, j2, j3 = ", j0, j1, j2, j3)
                if j0 >= 0 and j1 >= 0 and j2 >= 0 and j3 >= 0:
                    Wp_Indx.append([j0, j1, j2, j3])
                    Wp_Ops.append(["z", "z", "z", "z"])  # Z0 Z1 Z2 Z3, the order of operators in the plaquette
            ...
            # Ws2 operator with lower-left corner on sites 2, 6, 10, ... 
                ...
                j1 = self.Lat.nn_[i, 2]
                j3 = self.Lat.nn_[i, 1] 
                print("[Ws2 sites] j1, j2, j3 = ", j1, j2, j3)
                if j2 >= 0 and j3 >= 0 and j3 >=0:
                    Ws2_Indx.append([j1, j2, j3])
                    Ws2_Ops.append(["y", "x", "z"]) 
...
if __name__ == '__main__':
    ...
    lat = Lattice(param)
    test = Kitaev_Diamond_Chain(lat, param)
    ...
    rfile.close()
    # gs = 0.2 * evecs[:, 1] + 0.8 * evecs[:,0]  # ground state is a superposition of the first two eigenstates
    # gs = gs / np.linalg.norm(gs)  # normalize the state
    gs = evecs[:, 0] 

    Model = Kitaev_Diamond_Chain(Lat, para)
    E0 = matele(gs, Model.Ham, gs)
    print("E0 = ", E0)
