...
class Hamiltonian:
    ...
    def __init__(self, Lat):
        ...
        if self.Model != "Bose_Hubbard":
            self.Hx = Lat.Hx
            self.Hy = Lat.Hy
            self.Hz = Lat.Hz

        if self.Model != "AKLT" and self.Model != "Bose_Hubbard":
        ...
        elif self.Model == "Bose_Hubbard":
            self.Nsite = Lat.Nsite
            self.t = Lat.t
            self.U = Lat.U
            self.mu = Lat.mu
            self.maxOccupation = Lat.maxOccupation
    ...
    def BuildHubbard(self):
        ...
        print("[Hamiltonian.py] Building Hamiltonian as Sparse Matrix...")
        Boson = Dofs("Boson", self.maxOccupation)
        annih = Boson.annih
        creat = Boson.creat
        occup = Boson.occup
        Id = Boson.I
        ...
        # hopping term
        Ham = TwoSpinOps(self.tPair_, self.tcoef_, creat, annih, self.Nsite)
        Ham += np.transpose(Ham.conj())
        # chemical potential
        for i in range(0, self.Nsite):
            ida = sp.eye(self.maxOccupation ** i)
            idb = sp.eye(self.maxOccupation ** (self.Nsite - i - 1))
            Ham += sp.kron(ida, sp.kron(occup, idb)) * self.mu
        # on-site interaction
        for i in range(0, self.Nsite):
            ida = sp.eye(self.maxOccupation ** i)
            idb = sp.eye(self.maxOccupation ** (self.Nsite - i - 1))
            Ham += sp.kron(ida, sp.kron(occup.dot(occup - Id), idb)) * self.U
        return Ham
